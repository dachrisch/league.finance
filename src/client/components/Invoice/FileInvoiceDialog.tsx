...
interface FileInvoiceDialogProps {
  open: boolean;
  invoiceId: string;
...
export function FileInvoiceDialog({
  open,
  invoiceId,
  recipientName,
  totalPrice,
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      // Call tRPC fileInvoiceInDrive mutation
      const response = await fetch('/trpc/finance.invoicesDrive.fileInvoiceInDrive', {
        method: 'POST',
      ...
      pollIntervalRef.current = setInterval(async () => {
        ...
        try {
          // getInvoiceDriveStatus is a tRPC query; over HTTP it must be a GET
          // with the input in the ?input= query param (a POST returns 405).
          const statusInput = encodeURIComponent(JSON.stringify({ invoiceId }));
          const statusResponse = await fetch(
            `/trpc/finance.invoicesDrive.getInvoiceDriveStatus?input=${statusInput}`
          );
      ...
      setIsLoading(false);
      onError(err.message || 'Failed to file invoice');
    }
  }, [invoiceId, selectedFolderId, onError, onSuccess, onClose]);
    ...
    <div style={{
      ...
      <div className="card" style={{ width: '100%', maxWidth: '500px', maxHeight: '90vh', overflow: 'auto', padding: 'var(--spacing-xl)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-lg)' }}>
          <h2 style={{ margin: 0, fontSize: 'var(--font-size-xl)' }}>File invoice in Drive</h2>
