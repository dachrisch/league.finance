...
## Task 17: Wire Sheets sync into `invoices.create` and `FileInvoiceJob`
...
```ts
  ...
  it('still succeeds when the Sheets sync call rejects', async () => {
    vi.mocked(Invoice.findById).mockResolvedValue(makeInvoice() as any);
...
git commit -m "feat: sync created invoices and status changes to the Sheets ledger"
```
